function ColorCurve(a,b){this.points=[];this.currentpoint=-1;this.c=document.getElementById(a);this.ctx=this.c.getContext("2d");this.height=this.c.height;this.width=this.c.width;this.redraw=0;this.values=[];this.rgb=[];this.onchange=b;if(this.height!=this.width)console.log("ERROR: Canvas must have same width and height.");else{this.points.push({x:0,y:0});this.points.push({x:0.25,y:0.25});this.points.push({x:0.75,y:0.75});this.points.push({x:1,y:1});var c=this;this.c.onmousedown=function(a){c.MouseDown(a)};
this.c.onmouseup=function(a){c.MouseUp(a);c.Draw()};this.c.onmouseout=function(a){c.MouseUp(a);c.Draw()};this.c.onmousemove=function(a){c.MouseMove(a);1==c.redraw&&(c.Draw(),c.redraw=0)};this.Draw();this.UpdateValues()}}ColorCurve.prototype.UpdateValues=function(){this.rgb.splice(0,this.rgb.length);for(var a=0;256>a;a++)this.rgb.push(Math.round(255*this.GetY(a/255)));this.onchange()};ColorCurve.prototype.IsEqual=function(a,b){return a.x==b.x&&a.y==b.y?!0:!1};
...
ColorCurve.prototype.Quadratic=function(a,b,c,d){this.ctx.strokeStyle="#ffffff";this.ctx.lineWidth=1.5;var e=0;x0=b.x;x3=c.x;y0=b.y;y3=c.y;dx=x3-x0;dy=y3-y0;x1=2*x0/3+x3/3;x2=x0/3+2*x3/3;this.IsEqual(a,b)&&this.IsEqual(c,d)&&(y1=y0+dy/3,y2=y0+2*dy/3);this.IsEqual(a,b)&&!this.IsEqual(c,d)&&(e=(d.y-y0)/(d.x-x0),y2=y3-e*dx/3,y1=y0+(y2-y0)/2);!this.IsEqual(a,b)&&this.IsEqual(c,d)&&(e=(y3-a.y)/(x3-a.x),y1=y0+e*dx/3,y2=y3+(y1-y3)/2);!this.IsEqual(a,b)&&!this.IsEqual(c,d)&&(e=(y3-a.y)/(x3-a.x),y1=y0+e*dx/
3,e=(d.y-y0)/(d.x-x0),y2=y3-e*dx/3);this.ctx.beginPath();this.ctx.moveTo(x0*this.width,this.height-y0*this.height);step=(x3-x0)/20;tx=x0;for(a=0;1.05>=a;a+=0.05)ty=y0*Math.pow(1-a,3)+3*y1*Math.pow(1-a,2)*a+3*y2*(1-a)*a*a+y3*a*a*a,this.ctx.lineTo(tx*this.width,this.height-ty*this.height),this.values.push({x:tx,y:ty}),tx+=step;this.ctx.moveTo(0,this.height-this.points[0].y*this.height);this.ctx.lineTo(this.points[0].x*this.width,this.height-this.points[0].y*this.height);this.ctx.moveTo(this.points[3].x*
